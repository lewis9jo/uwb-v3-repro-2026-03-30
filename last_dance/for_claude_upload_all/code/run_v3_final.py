"""End-to-end v3 NoAttention pipeline (preprocess -> train -> evaluate -> figure).

This script shows the full experimental flow for Proposed_V2_NoAttention:
1) load & preprocess raw data (Lab or Classroom)
2) build train/val/test splits and check feature cleanup
3) train model (or load previously saved weights)
4) run prediction, evaluate metrics, save errors
5) generate CDF and t-SNE figures

Defaults are aligned with existing v3 artifacts.
"""
...
from new_bench_03.data.dataset import UWBDataset
...
RESULTS_CSV = OUTPUT_ROOT / "pipeline_results.csv"
...
def _trace_preprocessing(provider: DataProvider) -> None:
    # Optional verbose trace: call private methods to inspect how rows are transformed.
    # This does not change the training output; it only prints intermediate counts.
    ...
    print(
        f"[trace] after_filter_rows={len(filtered)} "
        f"(filter={provider.filter_type})"
    )
...
def _extract_features(bundle, split_name: str, profile: str) -> np.ndarray:
...
def _predict_with_shared(
    ...
    if getattr(model, "artifacts", None) is None:
        raise RuntimeError("Model not initialized (call fit or _init_state first).")
...
def plot_cdf(errors: np.ndarray, extra_errors: Dict[str, np.ndarray], save_dir: Path, task_name: str) -> None:
    ...
    def _plot(name: str, arr: np.ndarray, color: str) -> None:
        if arr.size == 0:
            return
        s = np.sort(arr)
        cdf = np.linspace(0, 1, len(s), endpoint=True)
        plt.plot(s, cdf, label=name, color=color, linewidth=1.2)
    ...
    for name, arr in extra_errors.items():
        if name == "Proposed_V3":
            ...
            color = "#333333"
        _plot(name, arr.astype(float), color)
...
def _subsample_tsne(features: np.ndarray, labels: np.ndarray, per_class: int, max_points: int) -> Tuple[np.ndarray, np.ndarray]:
...
def plot_tsne(
    ...
    for cls, name in class_names.items():
        ...
        if not np.any(mask):
            continue
        plt.scatter(emb[mask, 0], emb[mask, 1], s=8, alpha=0.7, label=name, color=class_colors.get(cls))
...
def run_task(task_key: str, task: dict, args: argparse.Namespace) -> Dict[str, float]:
    set_seed(args.seed)
    ...
    # 5) evaluate
    metrics = evaluate_and_pack(test_split.y_pos, test_split.y_class, pred_pos, pred_cls, pred_conf)
    ...
    # keep a stable seed-specific checkpoint copy
    ckpt_src = (
    ...
    if ckpt_src and ckpt_src.exists():
        shutil.copy2(ckpt_src, ckpt_dst)

    # 7) figures
    cdf_errors = {"Proposed_V3": errors_cm}
    if task_key == "classroom" and args.classroom_baseline:
        for name, path in BASELINE_CLASSROOM.items():
            if path.exists():
                cdf_errors[name] = np.load(path).astype(float)
            else:
                print(f"[warn] missing baseline: {path}")
    plot_cdf(errors_cm, cdf_errors, task_dir, task_key)
    plot_tsne(
        shared,
        test_split.y_class,
        task["class_labels"],
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="v3 full pipeline: no-attention model")
    parser.add_argument("--tasks", nargs="+", default=["all"], choices=["all", "lab", "classroom"], help="Run lab / classroom / all")
    ...
    parser.add_argument("--batch-size", type=int, default=128, help="Training/test batch size")
    ...
    parser.add_argument("--min-epochs", type=int, default=60, help="Min epochs before early stop")
    ...
    parser.add_argument("--rebuild-data", action="store_true", help="Rebuild dataset cache from raw files")
    parser.add_argument(
        "--classroom-baseline",
        action="store_true",
        help="For classroom task, add old baseline CDF overlays (LSTM/FC-SVM/EKF)",
    )
    parser.add_argument(
        "--trace-data",
        action="store_true",
...
def main() -> None:
    ...
    for task_key in requested:
        rows.append(run_task(task_key, TASKS[task_key], args))
